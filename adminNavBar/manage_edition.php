...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $newedition = $_POST['new_edition'];

    // Update the user's city in the database
    $sql = "UPDATE repos_data SET volume = '$newedition' WHERE id = '$userId'";

    if ($conn->query($sql) === TRUE) {
        $message = 'Edition updated successfully.';
    } else {
        $message = 'Error updating Edition: ' . $conn->error;
    }
...
<body>
    ...
    <div class="content">
        ...
        <form method="POST" action="manage_edition.php">
            <label for="user_id">User ID:</label>
            <input type="text" id="user_id" name="user_id" required>

            <label for="new_edition">New Edition:</label>
            <input type="text" id="new_edition" name="new_edition" required>
    ...
    $sql1 = " SELECT ID, author, volume FROM repos_data";
    $result1 = $conn1->query($sql1);
    if($result1->num_rows > 0){
        echo "<h1>Show All Editions!</h1>";
        echo "<table>
        <tr>
            <th>ID:</th>
            <th>Author:</th>
